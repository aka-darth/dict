...
$old_word=mysqli_fetch_assoc(mysqli_query($mysqli,"SELECT word,lang,target FROM dt_W_".$user['id']." WHERE id=".intval($_POST['id'])));
...
while($_POST['trans'.$i]){
	if($_POST['id'.$i]=="new"){
		$query=mysqli_query($mysqli,"SELECT id,target FROM dt_W_".$user['id']." WHERE word LIKE '".$_POST['trans'.$i]."'");
		if($finded_word=mysqli_fetch_assoc($query)){
			if($key=array_search($finded_word['id'],$old_targets)){
				...
				if(strlen($finded_word['target'])){
					mysqli_query($mysqli,"UPDATE dt_W_".$user['id']." SET target='".$finded_word['target'].",".$_POST['id']."' WHERE id=".$finded_word['id']);
				}else{
					mysqli_query($mysqli,"UPDATE dt_W_".$user['id']." SET target='".$_POST['id']."' WHERE id=".$finded_word['id']);
				}
				$new_targets[]=$finded_word['id'];
			}
		}else{
			//echo $_POST['trans'.$i]." нет в базе!<br>";
			$query=mysqli_query($mysqli,"INSERT INTO dt_W_".$user['id']." VALUES ('','".$_POST['trans'.$i]."','".$_POST['lang'.$i]."','0','','0','0','".$_POST['id']."')");
			$new_targets[]=mysqli_insert_id($mysqli);
...
mysqli_query($mysqli,$main_query);
...
if($err){
	...
	$line=mysqli_fetch_assoc(mysqli_query($mysqli,$query));
	...
	foreach($targets as $target){
		$word=mysqli_fetch_assoc(mysqli_query($mysqli,"SELECT id,word,lang FROM  dt_W_".$user['id']." WHERE id=".$target));
